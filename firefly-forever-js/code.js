...
let numberOfQuotes = (name) => {
  ...
  let quoteNumber = quotes[index].length;
  return quoteNumber;
};


let whoSaidThis = (quote) => {
 let phrase = quotes.indexOf(quote)
...
let longestQuote = (name) => {
  let index = crewMembers.indexOf(name);
  let quoteSize = quotes[index].sort(function(a, b){
...
let averageQuoteLength = (name) => {
  ...
  quotes[index].forEach(function(i) {
    quotelength += i.length;
  });
  let average = quotelength / quotes[index].length;
  let max = Math.round(average);
  return max;
};
