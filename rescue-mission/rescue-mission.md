...
* When creating a migration in Rails you can use a Rails generator command rather than `bundle exec rake db:create_migration`. The format of the command is `bundle exec rails generate migration <name_of_migration>`. For example, to create a migration to add a `questions` table you might run:

```no-highlight
$ bundle exec rails generate migration create_questions
